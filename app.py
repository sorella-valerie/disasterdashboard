...
# Page configuration (no icons or emojis)
st.set_page_config(
...
def _presence_mask(df: pd.DataFrame, col_name: str) -> pd.Series:
    """Return a 1-D boolean mask: True if the row has a present value in any duplicate column named col_name."""
...
def main():
        ...
        with c1:
            with st.container(border=True):
                ...
                if not coverage_df.empty:
                    ...
                    st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False}, key="summary_coverage_by_category")
                else:
        ...
        with c2:
            with st.container(border=True):
                ...
                if not heatmap_df.empty:
                    ...
                    st.plotly_chart(fig_heatmap, use_container_width=True, config={"displayModeBar": False}, key="summary_heatmap")
                else:
        ...
        with c3:
            with st.container(border=True):
                ...
                if not region_cov_df.empty:
                    ...
                    st.plotly_chart(fig_region, use_container_width=True, config={"displayModeBar": False}, key="summary_avg_by_region")
                else:
    ...
    # -----------------------
        ...
        # Find all dataframes containing any of the target categories
        dfs_with_category = []
            ...
            for tab, (name, df) in zip(tabs, dfs_with_category):
                with tab:
                    for cat in selected_categories:
                        matching_cols = [col for col in df.columns if cat in str(col)]
                            ...
                            with col_b:
                                if not non_empty.empty:
                                    ...
                                    st.plotly_chart(
                                        ...
                                        key=f"deepdive_{name}_{cat}_mini_chart"
                                    )
                            ...
                            if not non_empty.empty:
                                with st.expander(f"View data for {len(non_empty)} states"):
                                    ...
                                    st.dataframe(
                                        non_empty[show_cols],
                                        use_container_width=True,
                                        key=f"deepdive_{name}_{cat}_df"
                                    )
        else:
            st.warning("No datasets contain the target categories.")
    ...
    st.markdown(
        ...
        Emergency Management Dashboard<br>
